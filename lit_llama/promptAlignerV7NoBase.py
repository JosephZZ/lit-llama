...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig, aligner_embedding=None) -> None:
        ...
        self.aligner_mlp3 = SimpleTwoLayerMLP(config.n_embd, config.n_embd // config.n_head, config.n_embd // config.n_head)
    ...
    def forward(
        ...
        if self.current_model_mode == "aligner":
            ...
            align_embedding = self.aligner_mlp3(self.aligner_rms3(x+align_embedding))
            #repeat the head to be the size of the whole embedding
            final = torch.repeat_interleave(align_embedding, repeats=self.config.n_head, dim=0)
            return final, (align_embedding, self.aligner_gating_factor, self.initial_input_aligner_embedding) #, self.aligner_embedding_bias
