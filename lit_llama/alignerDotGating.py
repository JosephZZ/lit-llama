...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.block_idx >= self.adapter_start_layer:
            ...
            ay = scaled_dot_product_attention_handling_len_one_sigmoid(q, ak, av, attn_mask=amask, dropout_p=0.0, is_causal=False) # (B, nh, T, hs)
            y = y + ay
...
def scaled_dot_product_attention_handling_len_one_sigmoid(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=True):
    if k.shape[-2] == 1:
        # if the length of the key is 1, we can't do the softmax, so we do sigmoid instead
        attention_logit = torch.matmul(q, k.transpose(-1, -2))
        embedding_size = q.size(-1)
        attention_logit = attention_logit / (embedding_size ** 0.5)
        attention = torch.sigmoid(attention_logit)
        final = torch.matmul(attention, v)
    else:
        final = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout_p=dropout_p, is_causal=is_causal)
    return final
