...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int) -> None:
        ...
        if block_idx >= config.aligner_start_layer:
            self.gating_factor_aligner = None
            self.aligner_embedding = None #to be fed


    def _set_model_mode(self, is_aligner : bool, aligner_embedding:torch.tensor=None, aligner_gating_factor:torch.tensor=None):
        if is_aligner:
            self.current_model_mode = "aligner"
            self.aligner_embedding = aligner_embedding
            self.gating_factor_aligner = aligner_gating_factor
            self.adapter_start_layer = self.aligner_start_layer
        else:
            self.current_model_mode = "aligner_generator"
            self.aligner_embedding = None
...
class SimpleMLP(nn.Module):
    def __init__(self, config, output_dim ):
        ...
        self.linear1 = nn.Linear(config.n_embd, head_size)
        
        # Second linear layer
        self.linear2 = nn.Linear(head_size, output_dim)

    def forward(self, x):
        # First linear layer followed by ReLU activation
        x = F.tanh(self.linear1(x))
        
        # Second linear layer followed by ReLU activation
        x = self.linear2(x)
...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig, aligner_embedding=None) -> None:
        ...
        self.initial_input_aligner_embedding = nn.Parameter(torch.randn(config.n_embd) / (config.n_embd ** 0.5))

        self.aligner_mlp_bias1 = nn.Parameter(torch.zeros(config.n_embd))
        self.aligner_rms1 = RMSNorm(config.n_embd)
        self.aligner_mlp1 = MLP(config)

        # self.aligner_mlp_bias2 = nn.Parameter(torch.zeros(config.n_embd))
        # self.aligner_rms2 = RMSNorm(config.n_embd)
        # self.aligner_mlp2 = MLP(config)

        self.aligner_rms3 = RMSNorm(config.n_embd)
        self.aligner_mlp3 = SimpleMLP(config, 10)

        self.aligner_gating_factor = nn.Parameter(torch.zeros(1))#using it as the gating factor during align infer stage

        self.aligner_embd_seed_to_true_proj = nn.Linear(10, config.n_embd // config.n_head, bias=True)
        #initialize with normal distribution
        # self.pretrained_aligner_embedding_base = nn.Parameter(torch.empty(config.n_embd)) #nn.Parameter(torch.randn(config.n_embd))  #0.05 is the square root of n_embd

        self.set_model_mode(is_aligner=False)

    def init_suffix_embd_with_token(self):
        # init as token correspond to "summary"
        self.initial_input_aligner_embedding.data = self.transformer.wte(torch.tensor([15387],device=self.initial_input_aligner_embedding.device)).data
    ...
    def forward(
        ...
        B, T = idx.size()
        if self.current_model_mode == "aligner_generator":
            T = T+1 # add one for the initial input
        ...
        # concat the initial input embedding
        if self.current_model_mode == "aligner_generator":
            x = torch.cat([x, self.initial_input_aligner_embedding.expand(B, 1, -1)], dim=1)
        ...
        if self.current_model_mode == "aligner":
            ...
            x = x[-1,-1,:] #take the last output to be our aligner embedding, assuming batch size 1
            align_embedding = self.aligner_mlp1(self.aligner_rms1(x))
            #generate only one head
            align_embedding_seed = self.aligner_mlp3(self.aligner_rms3(x+align_embedding))
            final_embedding = self.aligner_embd_seed_to_true_proj(align_embedding_seed)
            #repeat the head to be the size of the whole embedding
            final_full = torch.repeat_interleave(final_embedding, repeats=self.config.n_head, dim=0)
            return final_full, (align_embedding_seed, self.aligner_gating_factor, self.initial_input_aligner_embedding) #, self.aligner_embedding_bias

    def set_model_mode(self, is_aligner:bool, aligner_embedding:torch.Tensor=None):
        if self.current_model_mode == "aligner" and is_aligner:
            return
        if self.current_model_mode == "aligner_generator" and not is_aligner:
            return
        
        if is_aligner:
            enable_original_forward(self.transformer.h)

            for block in self.transformer.h:
                block.attn._set_model_mode(is_aligner, aligner_embedding, self.aligner_gating_factor )

            self.current_model_mode = "aligner"
        else:
            enable_custom_forward(self.transformer.h)
...
def enable_original_forward(module):
    for layer in module.modules():
        if isinstance(layer, nn.Linear) :
            if hasattr(layer, 'is_lora') and layer.is_lora:
                continue
            setattr(layer, 'forward', layer.original_forward_method)
            layer.forward_method_type = "original"

def enable_custom_forward(module):
    for layer in module.modules():
        if isinstance(layer, nn.Linear) and (layer.weight.shape[0] != 8 or layer.weight.shape[1] != 8):
            if hasattr(layer, 'is_lora') and layer.is_lora:
                continue
            try:
                setattr(layer, 'forward', layer.custom_forward)
            except:
                init_adapter_v2_linear_with_bias_and_scale(layer)
            layer.forward_method_type = "custom"

def init_adapter_v2_linear_with_bias_and_scale(layer):
    layer.lora_linear1 = nn.Linear(layer.weight.shape[1], 8, bias=True)
    layer.lora_linear1.is_lora = True
    layer.lora_linear2 = nn.Linear(8, layer.weight.shape[0], bias=True)
...
def adapter_v2_new_forward(self, input: torch.Tensor) -> torch.Tensor:
    return F.linear(input, self.weight, self.bias)  \
            + self.lora_linear2(self.lora_linear1(input))
