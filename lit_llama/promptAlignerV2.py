...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig, aligner_embedding=None) -> None:
        ...
        self.aligner_embedding_bias = nn.Parameter(torch.zeros(config.n_embd)) #nn.Parameter(torch.randn(config.n_embd))  #0.05 is the square root of n_embd
        # self.aligner_embedding_bias = nn.Embedding(1,config.n_embd)
    ...
    def forward(
        ...
        if self.current_model_mode == "aligner":
            ...
            align_embedding = align_embedding + self.aligner_embedding_bias 
            return align_embedding #, self.aligner_embedding_bias
