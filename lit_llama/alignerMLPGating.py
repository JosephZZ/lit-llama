...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int, global_value_embedding=None, global_gating_mlp=None) -> None:
        super().__init__()
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            if global_value_embedding is not None:
                ...
                self.global_gating_mlp = global_gating_mlp
    ...
    def forward(
        ...
        if self.block_idx >= self.adapter_start_layer:
            ...
            expanded_ak = ak.expand(-1, -1, T, -1) # (B, nh, T, hs)
            concated_qk = torch.cat([q, expanded_ak], dim=-1) # (B, nh, T, 2*hs)
            gating_factor = self.global_gating_mlp(concated_qk) # (B, nh, T, 1)
            y = y + gating_factor*av # (B, nh, T, hs) #same as torch.matmul(gating_factor, av)
...
class Block(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int, global_value_embedding=None, global_gating_mlp=None) -> None:
        ...
        self.attn = CausalSelfAttention(config, block_idx, global_value_embedding, global_gating_mlp)
...
class MLP_Gating(nn.Module):
    def __init__(self, head_size) -> None:
        super().__init__()
        n_hidden = 4 * head_size
        self.c_fc1 = nn.Linear(2*head_size, n_hidden, bias=False)
        self.c_fc2 = nn.Linear(2*head_size, n_hidden, bias=False)
        self.c_proj = nn.Linear(n_hidden, 1, bias=False)
...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig) -> None:
        ...
        self.global_gating_mlp = MLP_Gating(config.n_embd // config.n_head)

        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
        self.transformer = nn.ModuleDict(
            dict(
                wte=nn.Embedding(config.vocab_size, config.n_embd),
                h=nn.ModuleList(Block(config, i, self.global_value_embedding, self.global_gating_mlp) for i in range(config.n_layer)),
...
def mark_only_adapter_as_trainable(model: LLaMA) -> None:
    ...
    for name, param in model.named_parameters():
        param.requires_grad = "adapter_wte" in name or "gating" in name or "global_value_embedding" in name
        if param.requires_grad:
            print("Training", name)

def adapter_state_from_state_dict(state_dict: dict) -> dict:
    """Returns the model state dict with only the adapter weights for saving."""
    return {name: param for name, param in state_dict.items() if "adapter_wte" in name or "gating" in name or "global_value_embedding" in name}
