...
from lit_llama.model import build_rope_cache, apply_rope, MLP, KVCache, RoPECache
...
class RMSNorm(nn.Module):
    ...
    def __init__(self, size: int, dim: int = -1, eps: float = 1e-5) -> None:
        ...
        self.eps = eps
        self.dim = dim
    def norm_only(self, x):
        norm_x = torch.mean(x * x, dim=self.dim, keepdim=True)
        x_normed = x * torch.rsqrt(norm_x + self.eps)
        return x_normed
    
    def scale_only(self, x_normed):
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        #     else:
        #         prefix = self.rotation_theta.weight.reshape(1, self.adapter_prompt_length, self.n_embd)
        #         aT = prefix.size(1)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.config.rotation_place == "normedX_to_attention_before_rescale":
                
            y = self.rms_1.norm_only(x)
            if self.block_idx >= self.config.adapter_start_layer:
                rotation = torch.stack([torch.cos(self.rotation_theta), torch.sin(self.rotation_theta)], dim=-1)
                y = y.view(B, T, 1, C)
                y = apply_rope(y, rotation.unsqueeze(0).expand(T,-1, -1))
                y = y.view(B, T, C)
            y = self.rms_1.scale_only(y)
        else:
            y = self.rms_1(x)
        h, new_kv_cache, new_adapter_kv_cache = self.attn(
            y, rope, mask, max_seq_length, input_pos, kv_cache, adapter_kv_cache
        )
        x = x + h

        y = self.rms_2(x)
        if self.config.rotation_place == "normedX_to_mlp":    
            if self.block_idx >= self.config.adapter_start_layer:
                rotation = torch.stack([torch.cos(self.rotation_theta), torch.sin(self.rotation_theta)], dim=-1)
                y = y.view(B, T, 1, C)
                y = apply_rope(y, rotation.unsqueeze(0).expand(T,-1, -1))
