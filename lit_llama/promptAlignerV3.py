...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig, aligner_embedding=None) -> None:
        ...
        self.initial_input_aligner_embedding = nn.Parameter(torch.zeros(config.n_embd))
        ...
        self.aligner_residue_gating_factor = nn.Parameter(torch.zeros(1))
    ...
    def forward(
        ...
        if self.current_model_mode == "aligner":
            ...
            align_embedding = x[-1,-1,:] #take the last output to be our aligner embedding, assuming batch size 1
            align_embedding = align_embedding + self.aligner_mlp_bias1
            align_embedding = self.aligner_mlp1(self.aligner_rms1(align_embedding))

            align_embedding = align_embedding + self.aligner_mlp_bias2
            align_embedding = self.aligner_mlp2(self.aligner_rms2(align_embedding))

            final_embedding = align_embedding * self.aligner_residue_gating_factor + self.pretrained_aligner_embedding_base 
            return final_embedding, (align_embedding, self.aligner_residue_gating_factor, self.pretrained_aligner_embedding_base) #, self.aligner_embedding_bias
...
def init_adapter_v2_linear_with_bias_and_scale(layer):
    layer.adapter_bias = torch.nn.Parameter(torch.zeros(layer.weight.shape[0], device=layer.weight.device), requires_grad=True).to(layer.weight.dtype)
    layer.adapter_scale = torch.nn.Parameter(torch.ones(layer.weight.shape[0], device=layer.weight.device), requires_grad=True).to(layer.weight.dtype)
...
def mark_only_adapter_as_trainable(model: LLaMA) -> None:
    ...
    for name, param in model.named_parameters():
        param.requires_grad = "adapter" in name or "gating_factor" in name or "aligner" in name or "generator" in name
...
def adapter_state_from_state_dict(state_dict: dict) -> dict:
    ...
    return {name: param for name, param in state_dict.items() if  "adapter" in name or "gating_factor" in name or "aligner" in name or "generator" in name}
