...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int, global_value_embedding=None) -> None:
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            # else:
            self.local_adapter_wte = nn.Embedding(config.adapter_prompt_length, config.n_embd)
            self.global_adapter_wte = global_value_embedding
    ...
    def forward(
        ...
        if self.block_idx >= self.adapter_start_layer:
            if adapter_kv_cache is not None:
                ...
                prefix_local = self.local_adapter_wte.weight.reshape(1, self.adapter_prompt_length, self.n_embd)
                prefix_global = self.global_adapter_wte.weight.reshape(1, self.adapter_prompt_length, self.n_embd)
                #concat the two
                prefix = torch.cat((prefix_local, prefix_global), dim=1) # (1, 2 * aT, C)
