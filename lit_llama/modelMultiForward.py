...
class LLaMA(nn.Module):
    ...
    def forward(
        ...
        for j in range(self.config.num_of_forwards):
            if input_pos is None:  # proxy for use_cache=False
                ...
                if not self.kv_caches:
                    ...
                    self.kv_caches = [
                        ...
                        for _ in range(self.config.n_layer*self.config.num_of_forwards)
                    ]
                for i, block in enumerate(self.transformer.h):
                    x, self.kv_caches[i+(self.config.n_layer*j)] = block(x, rope, mask, max_seq_length, input_pos, self.kv_caches[i+(self.config.n_layer*j)])
