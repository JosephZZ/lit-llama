...
class LLaMA(llama.LLaMA):
    ...
    def forward(
        ...
        if self.current_model_mode == "aligner":
            ...
            align_embedding = x + self.aligner_mlp_bias1 + self.pretrained_aligner_embedding_base
            align_embedding = self.aligner_mlp1(self.aligner_rms1(align_embedding))

            align_embedding = x + align_embedding + self.aligner_mlp_bias2
            align_embedding = self.aligner_mlp2(self.aligner_rms2(align_embedding)) * self.aligner_residue_gating_factor

            final_embedding = torch.stack([align_embedding, self.pretrained_aligner_embedding_base ], dim=0)
