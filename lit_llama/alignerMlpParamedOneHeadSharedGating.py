...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int, global_value_embedding=None, global_gating_factor=None) -> None:
        super().__init__()
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            if global_value_embedding is not None:
                ...
                self.global_gating_factor = global_gating_factor
                # print("Using global value embedding for adapter at layer ", block_idx)
            else:
                self.adapter_wte = None #nn.Embedding(config.adapter_prompt_length, config.n_embd)
    ...
    def forward(
        ...
        if self.block_idx >= self.adapter_start_layer:
            if adapter_kv_cache is not None:
                ...
                if isinstance(self.adapter_wte, nn.Embedding):
                    prefix = self.adapter_wte.weight.reshape(1, self.adapter_prompt_length, self.n_embd)
                elif isinstance(self.adapter_wte, torch.Tensor):
                    prefix = self.adapter_wte.reshape(1, self.adapter_prompt_length, self.n_embd)
                elif isinstance(self.adapter_wte, nn.Module):
                    prefix = self.adapter_wte()
                    prefix = torch.repeat_interleave(prefix, repeats=self.n_head, dim=1)
                    prefix = prefix.reshape(1, self.adapter_prompt_length, self.n_embd)
            ...
            y = y + self.global_gating_factor*ay # (B, nh, T, hs)
...
class Block(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int, global_value_embedding=None, global_gating_factor=None) -> None:
        ...
        self.attn = CausalSelfAttention(config, block_idx, global_value_embedding, global_gating_factor)
...
class EmbeddingGenerator(nn.Module):
    def __init__(self, seed_len, config):
        super().__init__()
        self.global_value_embedding_seed = nn.Parameter(torch.randn(config.adapter_prompt_length, 10) / 10 ** 0.5) 

        head_size = config.n_embd // config.n_head
        # First linear layer
        self.linear1 = nn.Linear(seed_len, head_size)
        # Second linear layer
    def forward(self):
        # First linear layer followed by ReLU activation
        # x = F.tanh(self.linear1(self.global_value_embedding_seed))
        # Second linear layer followed by ReLU activation
        x = self.linear1(self.global_value_embedding_seed)
...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig) -> None:
        ...
        self.global_value_embedding_generator = EmbeddingGenerator(10, config)

        self.global_gating_factor = torch.nn.Parameter(torch.zeros(1))

        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
        self.transformer = nn.ModuleDict(
            dict(
                wte=nn.Embedding(config.vocab_size, config.n_embd),
                h=nn.ModuleList(Block(config, i, self.global_value_embedding_generator, self.global_gating_factor) for i in range(config.n_layer)),
