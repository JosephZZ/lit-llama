        ...
        if self.block_idx >= self.adapter_start_layer:
            if adapter_kv_cache is not None:
                ...
                if isinstance(self.adapter_wte, nn.Embedding):
                    prefix = torch.repeat_interleave(self.adapter_wte.weight, self.n_head, dim=1)
                elif isinstance(self.adapter_wte, torch.Tensor):
                    prefix = torch.repeat_interleave(self.adapter_wte, self.n_head, dim=1)
            ...
            y = y*(1-self.global_gating_factor) + self.global_gating_factor*ay # (B, nh, T, hs)
...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig) -> None:
        ...
        self.global_value_embedding = nn.Embedding(config.adapter_prompt_length, config.n_embd//config.n_head)
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                h=nn.ModuleList(Block(config, i, self.global_value_embedding, self.global_gating_factor) for i in range(config.n_layer)),
