...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int, global_value_embedding=None) -> None:
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            self.gating_factor = torch.nn.Parameter(torch.zeros(1, config.n_head, 1, config.adapter_prompt_length))
    ...
    def scaled_dot_product_attention_with_custom_gating(self, q, k, v, gating_factor, attn_mask=None, dropout_p=0.0, is_causal=True):
        ...
        gated_attention = gating_factor * attention
        final = torch.matmul(gated_attention, v)
