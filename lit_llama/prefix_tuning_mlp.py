...
class LLaMA(llama.LLaMA):
    def __init__(self, config: LLaMAConfig) -> None:
        ...
        self.prefix_embd_seed = nn.Parameter(torch.randn(config.prefix_length, config.n_embd // config.n_head) / config.n_embd ** 0.5)
        self.prefix_embd_mlp = SimpleMLP(config.n_embd // config.n_head, config.n_embd // 4, config.n_embd)
    # def init_prefix_embd_with_token(self):
    #     self.prefix_embd.data = self.transformer.wte(torch.tensor([15387],device=self.prefix_embd.device)).data
    ...
    def forward(
        ...
        prefix_embd = self.prefix_embd_mlp(self.prefix_embd_seed)
        prefix_embd = prefix_embd.unsqueeze(0).expand(B, -1, -1)
