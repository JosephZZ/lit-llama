            ...
            if self.block_idx >= self.adapter_start_layer:
                ...
                if adapter_kv_cache is not None:
                    ...
                    if isinstance(adapter_embedding, nn.Embedding):
                        prefix = adapter_embedding.weight.reshape(B, adapter_length, self.n_embd)
                    elif isinstance(adapter_embedding, torch.Tensor):
                        prefix = adapter_embedding.reshape(B, adapter_length, self.n_embd)
                    else:
                    ...
                    ak = ak.view(B, aT, self.n_head, head_size).transpose(1, 2) # (B, nh, aT, hs)
                    av = av.view(B, aT, self.n_head, head_size).transpose(1, 2) # (B, nh, aT, hs)
...
class LLaMA(llama.LLaMA):
    ...
    def __init__(self, config: LLaMAConfig, aligner_embedding=None) -> None:
        ...
        self.aligner_embedding_proj = nn.Linear(config.n_embd, config.n_embd)
        self.aligner_gating_factor =  0.04#nn.Parameter(torch.zeros(1))#using it as the gating factor during align infer stage

        self.set_model_mode(is_aligner=False)

    def init_suffix_embd_with_token(self):
        # init as token correspond to "summary"
        self.initial_input_aligner_embedding =  nn.Parameter(self.transformer.wte(torch.tensor([15387])).data.clone(), requires_grad=True)
        # self.initial_input_aligner_embedding.requires_grad = True
    ...
    def forward(
        ...
        if self.current_model_mode == "aligner":
            ...
            aligner_embedding = x[:,-1,:] # (B, n_embd)
            aligner_embedding = self.aligner_embedding_proj(aligner_embedding) # (B, n_embd)

            return aligner_embedding, ( self.aligner_gating_factor, self.initial_input_aligner_embedding) #, self.aligner_embedding_bias
...
def enable_custom_forward(module):
    for layer in module.modules():
        if isinstance(layer, nn.Linear) and (layer.weight.shape[0] != LORA_RANK or layer.weight.shape[1] != LORA_RANK):
            if hasattr(layer, 'is_lora') and layer.is_lora:
                continue
            try:
...
def init_adapter_v2_linear_with_bias_and_scale(layer):
    layer.lora_linear1 = nn.Linear(layer.weight.shape[1], LORA_RANK, bias=True)
    layer.lora_linear1.is_lora = True
    layer.lora_linear2 = nn.Linear(LORA_RANK, layer.weight.shape[0], bias=True)
...
def adapter_v2_new_forward(self, input: torch.Tensor) -> torch.Tensor:
    return F.linear(input, self.weight, self.bias)  \
            + (LORA_ALPHA/LORA_RANK)*self.lora_linear2(self.lora_linear1(input))
