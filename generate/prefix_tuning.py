...
from lit_llama.prefix_tuning import LLaMA
from lit_llama.utils import lazy_load, llama_model_lookup, quantization
from scripts.prepare_alpaca import generate_prompt


def main(
    prompt: str = "How to improve my time management skills?",
    input: str = "",
    aligner_path: Path = Path("out/prefix_tuning_token_init/lit-llama-2-lima/7B/10vector-lr9e-05bs32/optimizer-epoch-50.0-iter-049999.pth"),
    ...
    with lazy_load(pretrained_path) as pretrained_checkpoint, lazy_load(aligner_path) as adapter_checkpoint:
        ...
        with fabric.init_module(empty_init=True), quantization(mode=quantize):
            model = LLaMA.from_name(name, prefix_length=aligner_length)
    ...
    print ("aligner_length:", model.config.prefix_length)
