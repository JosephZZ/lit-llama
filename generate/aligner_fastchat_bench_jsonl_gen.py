...
def main(
    ...
    input: str = "",
    aligner_path: Path = Path("out/aligner/lit-llama-2-lima/7B/1vector-start_layer2-lr9e-05bs32/epoch-1.27996-iter-031999.pth"),
    ...
    with lazy_load(pretrained_path) as pretrained_checkpoint, lazy_load(aligner_path) as adapter_checkpoint:
        ...
        with fabric.init_module(empty_init=True), quantization(mode=quantize):
            model = LLaMA.from_name(name, aligner_length=aligner_length, aligner_start_layer=aligner_start_layer)
    ...
    print(f"aligner length:{model.config.adapter_prompt_length}")
    model.eval()
        ...
        for question in tqdm(questions):
            ...
            with open(os.path.expanduser(answer_file), "a") as fout:
                ans_json = {
                    ...
                    "model_id": aligner_path.name,  #get the model name
                    "choices": choices,
                    "tstamp": time.time(),
                }
