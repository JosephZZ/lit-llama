...
from lit_llama.alignerDeepReweight import LLaMA, LLaMAConfig, mark_only_adapter_as_trainable, adapter_state_from_state_dict
...
warmup_iters = 100 #1 * (epoch_size // micro_batch_size) // devices  # 2 alpaca epochs
start_iter = 0
...
if "beaver" in data_dir.name:
    out_dir = Path(f"out/alignerDeepReweight/{model_base}-{data_dir.name}/{model_version}-{safer_or_better}/{aligner_length}vector-start_layer{aligner_start_layer}-lr{learning_rate}bs{int(batch_size)}/")
else:
    out_dir = Path(f"out/alignerDeepReweight/{model_base}-{data_dir.name}/{model_version}/{aligner_length}vector-start_layer{aligner_start_layer}-lr{learning_rate}bs{int(batch_size)}/")
...
def train(
    ...
    aligner_path = os.path.join(out_dir, f"epoch-{epoch:.2f}.pth")
    optimizer_path = os.path.join(out_dir, f"optimizer-epoch-{epoch:.2f}.pth")
    save_model_checkpoint(fabric, model, optimizer, aligner_path, optimizer_path)
