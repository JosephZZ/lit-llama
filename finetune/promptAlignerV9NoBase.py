...
promptAligner_version = "V9.1NoBaseWithProj"
from lit_llama.promptAlignerV9NoBase import LLaMA, LLaMAConfig, mark_only_adapter_as_trainable, adapter_state_from_state_dict
...
warmup_iters = 100 #(0.5*epoch_size // micro_batch_size) // devices  # 2 alpaca epochs
...
eval_interval = 0.05 * epoch_size // (batch_size*devices)
save_interval = 0.2 * epoch_size // (batch_size*devices)  # x epochs
eval_iters = 100
log_interval = batch_size // micro_batch_size
...
aligner_generator_length = 1
aligner_generator_start_layer = 2

pretrained_path = project_home_dir / f"checkpoints/{model_base}/{model_version}/lit-llama.pth"

out_dir = project_home_dir / f"out/promptAligner{promptAligner_version}/{model_base}-{data_dir.name}{custom_outdir_suffix}/{model_version}/{aligner_length}vector-start_layer{aligner_start_layer}-lr{learning_rate}bs{int(batch_size)}warmup{warmup_iters}/"
...
def main():
    ...
    with fabric.init_module():
        ...
        model.init_suffix_embd_with_token()
    print("aligner length: ",model.config.aligner_length)

    mark_only_adapter_as_trainable(model)

    # if embd_proj_weights:
    #     embd_proj = torch.load(embd_proj_weights)
    #     model.aligner_embd_seed_to_true_proj.weight.data = embd_proj["global_value_embedding_generator.linear1.weight"]
    #     model.aligner_embd_seed_to_true_proj.bias.data = embd_proj["global_value_embedding_generator.linear1.bias"]
    #     model.aligner_embd_seed_to_true_proj.weight.requires_grad = True
    #     model.aligner_embd_seed_to_true_proj.bias.requires_grad = True
...
def train(
    ...
    for iter_num in range(start_iter,max_iters):
        ...
        with fabric.no_backward_sync(model, enabled=((iter_num + 1) % gradient_accumulation_iters != 0)):
            ...
            aligner_embedding, other = model(system_align_prompt_ids) #suppose we use bf16, otherwise change it
        ...
        if iter_num % log_interval == 0:
            ...
            fabric.print(f"learnable suffix token:", other[1])
            fabric.print(f"gating factor of the embd:", other[0])
            with open(os.path.join(out_dir, "train log.txt"), "a") as file:
                file.write(f"iter {iter_num}: train loss {loss.item():.4f}\n")
...
@torch.no_grad()
def generate_response(model, instruction, system_prompt, input=""):
    tokenizer = Tokenizer("checkpoints/lit-llama-2/tokenizer.model")
    
    system_prompt_token = tokenizer.encode(system_prompt, bos=True, eos=False, device=model.device).unsqueeze(0)
    model.set_model_mode(is_aligner=False)
    aligner_embedding, _ = model(system_prompt_token) 
    model.set_model_mode(is_aligner=True, aligner_embedding=aligner_embedding.to(torch.bfloat16))
...
def validate(fabric: L.Fabric, model: torch.nn.Module, val_data: np.ndarray) -> torch.Tensor:
    ...
    # produce an example:
    system_prompt = "You are a movie critic. You are watching a movie and writing a review."
    instruction = "Recommend a movie for me to watch during the weekend and explain the reason."

    output = generate_response(model, instruction, system_prompt)

    fabric.print(instruction)
    fabric.print(output)

    with open(os.path.join(out_dir, "samples.txt"), "a") as file:
        file.write(f"\n###Instruction\n{instruction}\n###Response\n{output}\n\n")
