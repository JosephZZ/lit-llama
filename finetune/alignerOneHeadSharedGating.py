...
from lit_llama.alignerOneHeadSharedGating import LLaMA, LLaMAConfig, mark_only_adapter_as_trainable, adapter_state_from_state_dict
...
epoch_size = 169000  # train dataset size ; alpaca is 50000, isotonic is 280000, baize is 200000
num_epochs = 3
...
out_dir = Path(f"out/alignerOneHeadSharedGatingBugFixed/{model_base}-{data_dir.name}/{model_version}/{aligner_length}vector-start_layer{aligner_start_layer}-lr{learning_rate}bs{int(batch_size)}-weightDecay{weight_decay}-wu{warmup_epoch}/")
    ...
    for iter_num in range(start_iter,max_iters):
        ...
        if (iter_num + 1) % gradient_accumulation_iters == 0:
            ...
            if step_count % eval_interval == 0:
                ...
                fabric.print(f"epoch {epoch} step {iter_num}: val loss {val_loss:.4f}")
                fabric.barrier()
        ...
        if iter_num % log_interval == 0:
            ...
            with open(os.path.join(out_dir, "train_log.txt"), "a") as file:
                file.write(f"epoch {epoch} iter {iter_num}: train loss {loss.item():.6f}\n")
