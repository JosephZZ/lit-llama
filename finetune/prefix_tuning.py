...
from lit_llama.prefix_tuning import LLaMA, LLaMAConfig, mark_only_adapter_as_trainable, adapter_state_from_state_dict
...
epoch_size = 52000  # train dataset size ; alpaca is 50000, isotonic is 280000, baize is 200000
num_epochs = 5
...
out_dir = Path(f"out/prefix_tuning_token_init/{model_base}-{data_dir.name}/{model_version}/{prefix_length}vector-lr{learning_rate}bs{int(batch_size)}/")
save_model_name = f"{model_base}-{model_version}-{prefix_length}vector-lr{learning_rate}bs{int(batch_size)}.pth"
...
print(f"Training LLaMA-prefix_tuning with base model {model_base} using {model_version} parameters on the {data_dir.name} dataset, saving to {out_dir}")
...
def main():
    ...
    with fabric.init_module():
        ...
        if previous_prefix_path:
            model.load_state_dict(torch.load(previous_prefix_path), strict=False)
        else:
            model.init_prefix_embd_with_token()

    print("prefix length: ",model.config.prefix_length)
...
def get_batch(fabric: L.Fabric, data: list):
    ...
    max_len = min(max(len(s) for s in input_ids), max_seq_length-prefix_length)
