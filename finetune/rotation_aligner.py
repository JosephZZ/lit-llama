...
from lit_llama.rotation_aligner import LLaMA, LLaMAConfig, mark_only_adapter_as_trainable, adapter_state_from_state_dict
...
def main(
        ...
        data_dir: Path = Path("data/rolebench/eng_instruction_generalization_sheldon_5"),
    ...
    out_dir: Path = Path(f"out/rotation_aligner_all/rotate-{rotation_place}-{rotation_span}-globalRotation-{using_global_rotation}/{model_base}-{data_dir.name}/{model_version}/{aligner_length}vector-start_layer{aligner_start_layer}-lr{learning_rate}bs{int(batch_size)}weightDecay{weight_decay}wu{warmup_epoch}/")
