...
from lit_llama.alignerOneHeadDeepReweight import LLaMA, LLaMAConfig, mark_only_adapter_as_trainable, adapter_state_from_state_dict
...
epoch_size = 52000  # train dataset size ; alpaca is 50000, isotonic is 280000
num_epochs = 3
max_iters = num_epochs * (epoch_size // micro_batch_size) // devices
weight_decay = 0
max_seq_length = 2048 #alpaca 256, dolly 1024, lima 2048, isotonic 1536 # see scripts/prepare_alpaca.py
warmup_iters = 0.2 * (epoch_size // micro_batch_size) // devices  # 2 alpaca epochs
start_iter = 0
...
if "beaver" in data_dir.name:
    out_dir = Path(f"out/alignerOneHeadDeepReweight/{model_base}-{data_dir.name}/{model_version}-{safer_or_better}/{aligner_length}vector-start_layer{aligner_start_layer}-lr{learning_rate}bs{int(batch_size)}/")
else:
    out_dir = Path(f"out/alignerOneHeadDeepReweight/{model_base}-{data_dir.name}/{model_version}/{aligner_length}vector-start_layer{aligner_start_layer}-lr{learning_rate}bs{int(batch_size)}/")
    ...
    for iter_num in range(start_iter,max_iters):
        ...
        if (iter_num + 1) % gradient_accumulation_iters == 0:
            ...
            if step_count % save_interval == 0:
                ...
                aligner_path = os.path.join(out_dir, f"epoch-{epoch:.1f}-iter-{iter_num}.pth")
                optimizer_path = os.path.join(out_dir, f"optimizer-epoch-{epoch:.1f}-iter-{iter_num}.pth")
        ...
        if iter_num % log_interval == 0:
            ...
            with open(os.path.join(out_dir, "train log.txt"), "a") as file:
                file.write(f"epoch {epoch}: train loss {loss.item():.6f}\n")

    aligner_path = os.path.join(out_dir, f"epoch-{epoch:.2f}-iter-{iter_num}.pth")
    optimizer_path = os.path.join(out_dir, f"optimizer-epoch-{epoch:.2f}-iter-{iter_num}.pth")
    save_model_checkpoint(fabric, model, optimizer, aligner_path, optimizer_path)
...
def get_batch(fabric: L.Fabric, data: list):
    ...
    if "orca" in data_dir.name or "philo" in data_dir.name:
