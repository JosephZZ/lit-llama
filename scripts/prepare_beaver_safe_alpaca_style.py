...
IGNORE_INDEX = -1
home_dir = Path("/home/shuwen/ziheng/llm/lit-llama/")
def prepare(
    data_folder: Path = Path("data/beaver_safe2"), 
...
def fill_in_template(prompt, response):
...
def prepare_line(line, tokenizer: Tokenizer, max_length: int):
    ...
    """
    dialog_0 = fill_in_template(line["prompt"], line['response_0'])
    dialog_1 = fill_in_template(line["prompt"], line['response_1'])
