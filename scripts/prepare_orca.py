...
def prepare(
    data_folder: Path = Path("data/orca"), 
    ...
    if not os.path.exists(data_folder / DATA_FILE_NAME):
        ds = datasets.load_dataset("Open-Orca/OpenOrca")
        ds['train'].to_json(file_path)
    ...
    for item in data:
        ...
        sys_prompt_length.append(len(item["system_prompt"]))
...
def prepare_line(line: str, tokenizer: Tokenizer, max_length: int, max_sys_prompt_length: int):
    ...
    """
    system_align_prompt = tokenize(tokenizer, line['system_prompt'], max_length=max_sys_prompt_length, eos=False)
