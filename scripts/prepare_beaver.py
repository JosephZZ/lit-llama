...
def prepare(
    data_folder: Path = Path("data/beaver"), 
    ...
    # replace unicode with appropriate symbols to the entire dataset
    # data_test = [replace_unicode_escapes(entry) for entry in data_test]
    # data_train = [replace_unicode_escapes(entry) for entry in data_train]
...
def replace_unicode_escapes(obj):
    if isinstance(obj, dict):
        ...
        return [replace_unicode_escapes(element) for element in obj]
